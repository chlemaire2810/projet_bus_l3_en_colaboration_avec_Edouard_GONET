...
// UTILISER SI SOUS CODEBLOCKS/MAC
// #include "SDL.h"
// #include "SDL_image.h"
#include "SDL_VSYNC.h"
#include <stdio.h>
...
bool loadFromFile_LTexture(LTexture *l, SDL_Renderer *gRenderer, char *path)
{
    ...
    {
        // Color key image  -> permet de definir la couleur qui sera consid�r�e comme transparente, pour les operations de blit
...
void render_LTexture(LTexture *l, SDL_Renderer *gRenderer, int x, int y, int frame)
{
    ...
    // efface la fenetre -> fait � la main dans le main au final, cela �vite un effet "blanchi" sur les sprites
    // SDL_SetRenderDrawColor( gRenderer, 0xFF, 0xFF, 0xFF, 0xFF );
    // SDL_RenderClear( gRenderer );

    // Render to screen
    SDL_RenderCopy(gRenderer, l->mTexture, clip, &renderQuad); // Clip donne la partie de la texture � recopier sur le rendu
    // SDL_RenderCopy( gRenderer, l->mTexture, NULL, &renderQuad );  // NULL -> sur toute la fenetre ^^

    // maj � l'ecran de la texture -> fait � la main dans le main
    // SDL_RenderPresent(gRenderer);
}

// affiche le sprite � la pos (x,) et m�morise la pos dans sa structure

void Affiche_Sprite(LTexture *l, SDL_Renderer *gRenderer, int x, int y, int frame)
{
    // m�morise la position du sprite
    l->posX = x;
    l->posY = y;
...
{
    // permet d'affiche plusieurs fois le m�me sprite, pour effet visuel de glisser
...
{
    l->mTexture = mT; // OU SDL_RenderCopy(renderer, texture, NULL, &dst);  //recipie une texture d'unit� sur une autre (texture de tout le jeu)
}
...
bool init(SDL_Window **gWindow, SDL_Renderer **gRenderer)
{
    ...
    {
        printf("sdl has been initialised");
...
{
    // recopie 4 fois le m�me sprite, permet une utilisation de 'Affiche_Sprite' et 'Deplace_Sprite' (qui s'attendent � une texture comprenant 4 zones)
    ...
    else
    {
        ...
        //  dit autrement: UNE image contient UNE texture constitu�es de 4 zones (pour l'animation)
