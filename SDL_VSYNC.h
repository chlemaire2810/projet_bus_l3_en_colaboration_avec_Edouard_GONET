...
// UTILISER SI SOUS LINUX/MAC,VSC AVEC SDL2 INSTALLEE
#include <SDL2/SDL.h>
...
#define NUMBERFRAMESONSAMEPLACE 2  // nombre de fois que l'on redessine chaque sprite avant de le d�placer
...
typedef struct
{
    ...
    // position coin sup gauche o� est affich�e la texture dans la fenetre (renderer)
    int posX;
    int posY;

    // dist du prochain affichage   (posXarriv�eA - PosXdepartB)/NBIMAGESPOURLEDEPLACEMENT_A_VERS_B
    int incX;
    int incY;
} LTexture;
